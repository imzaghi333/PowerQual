...
window.onload = function(){
    ...
    function addTr(couts,rowid){
        ...
        for(var i=2; i<counts; i++){
            ...
            // ---------- End here ----------
            test_item.innerHTML+='<input name="test_order[]" id="test_order" type="text" style="width:20px;" />&nbsp;<select name="result['+rowid+rowCount+']" id="result['+rowid+rowCount+']" onchange="printMatrixResult('+rowid+','+rowid+rowCount+')">'+res+'</select>';
            if(i==counts-1){            
       ...
       // var add_del = row.insertCell(finlclum);
       // alert("finlclum="+finlclum);
       // add_del.innerHTML='<input type="button" name="add" class="btn_add" value="Add" />&nbsp;<input type="button" name="del" class="btn_del" value="Del" />';
       // cell3.innerHTML = "";
       // cell4.innerHTML = "";
        /*
        var html='';
        ...
        html+='<td><select name="test_item[]" id="test_item">';
        html+=tt;
        html+='</select></td>';
        
        for(var i=0; i<counts; i++){
            html+='<td><input name="test_order[]" id="test_order" type="text" /></td>';
        }
        couts=coutI+1;
...
function printResult(selecid,value,failcontent){
    ...
    if(txt.match(reg)){
        //alert("match fail");
       // window.open("fail.php?id=1","_blank","填写Fail的原因","height=500, width=700, top=100, left=100");
        window.open("fail.php?id="+value+"&selecid="+selecid,"填写Fail的原因","height=500, width=850, top=100, left=100");
    }
}
*/
...
function printMatrixResult(value,selecid){
    ...
    if(txt.match(reg)){
        //alert("match fail");
        // window.open("fail.php?id=1","_blank","填写Fail的原因","height=500, width=700, top=100, left=100");
        window.open("fail.php?id="+value+"&selecid="+selecid,"_blank","填写Fail的原因","height=500, width=800, top=100, left=100");
    }
}
